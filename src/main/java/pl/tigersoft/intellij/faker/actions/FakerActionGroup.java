...
public class FakerActionGroup extends ActionGroup {
    ...
    private static final List<String> excludedClasses = Arrays.asList("Number", "CreditCardType", "Faker", "Get");
    private static final List<String> excludedMethods = Arrays.asList("wait", "notify", "notifyAll", "toString", "getClass", "hashCode", "getFaker");
        ...
        return new ActionGroup(prettyName(method.getName()), true) {
            ...
            public AnAction[] getChildren(@Nullable AnActionEvent event) {
                return invokeMethod(method, faker, event)
                        .map(object -> Arrays.stream(object.getClass().getMethods())
